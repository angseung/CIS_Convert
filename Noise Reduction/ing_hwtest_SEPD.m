...
% cimg = imread('./input_images/lena512.bmp');
% img = imresize(cimg,[1080,1920]);
% imshow(uint8(img));
% sig = 0.07;
% nimg = imnoise(img,'salt & pepper',sig);
% input_image = nimg;
% save('input_image','input_image');
...
dbg_b = zeros(1920,1);
dbg_Dmean = zeros(1920,4);
dbg_Dsub  = zeros(1920,12);
dbg_ia    = zeros(1920,5);
row_buffer = zeros(1920,1);
dgb_readData = zeros(1920,3);
dbg_noise = zeros(1080,1920);
for i=2:rowsize+1
	for j=2:colsize+1
...
figure(2);
% White region is regarded as plane
imshow(uint8(dbg_noise));
