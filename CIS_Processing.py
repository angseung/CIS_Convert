...
def hw_RSEPD_fast(input_image = None, Ts = 20):
    ...
    row_buffer = np.zeros((rowsize,), dtype = np.float64)
    ...
    for i in range(colsize, (((rowsize - 1) * colsize) - 1), 1):
        ...
        if (x == (colsize - 2)):
            padIm[(y * rowsize) : ((y + 1) * rowsize)] = np.round(row_buffer, 0)
            row_buffer[:] = 0.0
